...
// --- **** WORD & SENTENCE LINKED LIST (Unchanged) **** ---
...
WordNode* insert_word_at(WordNode* doc_head, int sentence_index, int word_index, const char* content) {
    ...
    if (doc_head == NULL && sentence_index == 0 && word_index == 0) {
        // Note: We don't log here because the calling function (handle_client_request) does.
        // We also keep printf for debugging the *logic* of the write.
